...
actual class ImageStorage {
    ...
    actual suspend fun saveImage(bytes: ByteArray): String {
        return withContext(Dispatchers.Default) {
            ...
            // Direct access(Pointer) to memory
            val rawImageData = bytes.usePinned {
                NSData.create(
        ...
        return withContext(Dispatchers.Default) {
            memScoped {
                NSData.dataWithContentsOfFile(fileName)?.let { bytes ->
                    // we are fetching the file name from the actual pointer
                    val imageByteArray = ByteArray(bytes.length.toInt())
                    bytes.getBytes(imageByteArray.refTo(0).getPointer(this), bytes.length)
